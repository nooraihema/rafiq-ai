// chat.js v13.0 - The Harmonized Core compatible version
// Now fully asynchronous to work with the resilient storage.
...
// --- Configuration for v12.1 remains valid ---
...
// --- Dynamic Confidence Threshold Function (No changes needed) ---
...
// --- Helper: Build clarification prompt (No changes needed) ---
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ===== التعديل الأول: استخدام await هنا =====
    ...
    if (detectCritical(rawMessage)) {
      ...
      // ===== التعديل الثاني: استخدام await هنا =====
    ...
    if (profile.expectingFollowUp?.isClarification) {
      ...
      if (CLARIFICATION_VALID_CHOICES.test(trimmed)) {
        ...
        if (idx >= 0 && idx < (candidate.options || []).length) {
          ...
          // ===== التعديل الثالث: استخدام await هنا =====
          await saveUsers(users);
    ...
    if (topCandidates.length > 0) {
      ...
      if (bestCandidate.score >= confidenceThreshold) {
        ...
        if (secondCandidate && (bestCandidate.score - secondCandidate.score < AMBIGUITY_MARGIN)) {
          ...
          // ===== التعديل الرابع: استخدام await هنا =====
          await saveUsers(users);
        ...
        if (intent) {
          ...
          // ===== التعديل الخامس: استخدام await هنا =====
          await saveUsers(users);
    ...
    // ===== التعديل السادس: استخدام await هنا =====
    ...
    // ===== التعديل السابع: استخدام await هنا =====
    await saveUsers(users);
